...
import { getPaymentMethodLabel, getPaymentStatusLabel } from "../../utils/payments";
...
const OrderList = ({ item, onPress, testID }) => {
  const quantity = useMemo(
    () => item?.items?.reduce((total, product) => total + Number(product.quantity || 0), 0) || 0,
    [item]
  );
    ...
    <View style={styles.container} testID={testID}>
      ...
      <View style={styles.innerColumn}>
        <Text
          ...
          testID={testID ? `${testID}-delivery-status` : undefined}
        >
          Delivery : {formatDeliveryStatus(item?.status)}
        </Text>
        <Text
          style={styles.secondaryText}
          testID={testID ? `${testID}-payment-method` : undefined}
        >
          Payment Method : {getPaymentMethodLabel(item?.payment_type)}
        </Text>
        <Text
          style={styles.secondaryText}
          testID={testID ? `${testID}-payment-status` : undefined}
        >
          Payment Status : {getPaymentStatusLabel(item?.payment_status)}
        </Text>
      </View>
      <View style={styles.innerRow}>
        <TouchableOpacity
