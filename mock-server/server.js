...
app.post("/checkout", authMiddleware, (req, res) => {
  ...
  const selectedPaymentType = payment_type || "cod";
  const validPaymentTypes = ["cod", "mock_wallet"];
  const validPaymentStatuses = ["pending", "paid", "failed"];
  if (!validPaymentTypes.includes(selectedPaymentType)) {
    return res.status(400).json({ success: false, message: "Invalid payment type" });
  }
  const selectedPaymentStatus = payment_status || (selectedPaymentType === "mock_wallet" ? "paid" : "pending");
