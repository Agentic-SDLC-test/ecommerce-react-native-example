...
const PAYMENT_STATUS_BY_TYPE = { cod: "payment_due", wallet_mock: "paid" };
...
app.post("/checkout", authMiddleware, (req, res) => {
  ...
  if (!PAYMENT_TYPES.includes(payment_type) || payment_status !== PAYMENT_STATUS_BY_TYPE[payment_type]) {
    console.log(JSON.stringify({
      event: "checkout_rejected",
      payment_type,
      payment_status,
      fulfilment_status: status,
      reason: "invalid_payment_method_or_status",
    }));
    return res.status(400).json({ success: false, message: "Invalid payment method or payment status" });
  }
  ...
  console.log(JSON.stringify({
    ...
    payment_type,
    payment_status,
    fulfilment_status: newOrder.status,
  }));
